...
def _long_filter(width:int,height:int,index:int)->str:
    return (f"split=2[bg][fg];f"[bg]scale={width}:{height}:force_original_aspect_ratio=increase,crop={width}:{height},gblur=sigma=24,eq=brightness=-0.22:saturation=0.72[bg2];[fg]scale={width}:{height}:force_original_aspect_ratio=decrease[fg2];[fg2]eq=contrast=1.04:saturation=1.03[fg3];[bg2][fg3]overlay=(W-w)/2:(H-h)/2,setsar=1,format=yuv420p")
...
def _trim_or_loop_to_duration(src:Path,dest:Path,target_duration:float,long_form:bool=False,index:int=0)->None:
    ...
    if src_duration>=target_duration:
        start=0.5 if src_duration>target_duration+0.5 else 0.0;cmd=["ffmpeg","-y","-ss",str(start),"-i",str(src),"-t",str(target_duration),"-an","-vf",vf,"-c:v","libx264","-crf","18","-preset","veryfast","-pix_fmt","yuv420p","-r","30","-aspect",aspect,str(dest)]
...
def assemble(beats:list[dict],footage_paths:list[Path],work_dir:Path,out_path:Path,long_form:bool=False)->Path:
    ...
    if long_form:style="FontSize=15,PrimaryColour=&H00F5F5F5,OutlineColour=&H00101010,BorderStyle=1,Outline=1,Shadow=1,Alignment=2,MarginV=42,MarginL=160,MarginR=160";aspect="16:9"
    else:style="FontSize=18,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=1,Shadow=1,Alignment=2,MarginV=120,MarginL=110,MarginR=110";aspect="9:16"
