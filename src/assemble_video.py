...
def _long_filter(width:int,height:int,index:int)->str:
    # True 16:9 documentary treatment. Portrait/square archive sources remain
    # fully visible over a soft cinematic background instead of becoming Shorts-like.
    return (f"split=2[bg][fg];"
            f"[bg]scale={width}:{height}:force_original_aspect_ratio=increase,crop={width}:{height},gblur=sigma=24,eq=brightness=-0.22:saturation=0.72[bg2];"
            f"[fg]scale={width}:{height}:force_original_aspect_ratio=decrease[fg2];"
            f"[fg2]eq=contrast=1.04:saturation=1.03[fg3];"
            f"[bg2][fg3]overlay=(W-w)/2:(H-h)/2,setsar=1,format=yuv420p")
...
def _trim_or_loop_to_duration(src:Path,dest:Path,target_duration:float,long_form:bool=False,index:int=0)->None:
    ...
    if src_duration>=target_duration:
        ...
        cmd=["ffmpeg","-y","-ss",str(start),"-i",str(src),"-t",str(target_duration),"-an","-vf",vf,"-c:v","libx264","-crf","18","-preset","veryfast","-pix_fmt","yuv420p","-r","30","-aspect",aspect,str(dest)]
    else:
        cmd=["ffmpeg","-y","-stream_loop","-1","-i",str(src),"-t",str(target_duration),"-an","-vf",vf,"-c:v","libx264","-crf","18","-preset","veryfast","-pix_fmt","yuv420p","-r","30","-aspect",aspect,str(dest)]
    run(cmd)
