...
Copyright policy: the pipeline does not treat editing, cropping, narration, or
commentary as making third-party copyrighted material copyright-free. Assets
must be public-domain, CC/CC0, or otherwise explicitly rights-cleared/licensed.
"""
...
SAFE_COLLECTIONS={"usgovernmentfilms","NASAarchive","nasa","prelinger","universal_newsreels"}; PREFERRED_EXTENSIONS=(".mp4",".m4v",".mov"); MAX_VIDEO_BYTES=500*1024*1024; MAX_DOWNLOAD_RETRIES=3; MAX_CANDIDATES=15; SEARCH_TTL=7*24*3600; MAX_WIKIMEDIA_429=1
...
def _query_variants(query:str)->list[str]:
    ...
    q=query.lower()
    if "construction" in q and "tower" in q:variants += ["Eiffel Tower construction","Eiffel Tower workers","Paris 1889 Eiffel","Eiffel Tower historical photograph"]
    if any(x in q for x in ("apollo","moon","lunar")):variants += ["Apollo 11","Apollo astronauts","NASA Apollo","moon landing","Saturn V","Apollo mission","Apollo historical photograph"]
    if "berlin" in q and "wall" in q:variants += ["Berlin Wall","Berlin Wall 1989","Berlin Wall historical photograph"]
    if "titanic" in q:variants += ["Titanic","Titanic ship","Titanic historical photograph","Titanic passengers"]
...
def _candidate_items(query:str,fallback_query:str|None=None,cache_dir:Path|None=None):
    ...
    for q in queries:
        ...
        for doc in search_archive(q,cache_dir=cache_dir):
            ...
            relevance=max(_relevance(query,doc),_relevance(fallback_query or query,doc))
...
def _image_to_video(url:str,dest:Path,duration:float,index:int)->Path:
    ...
    r=requests.get(url,stream=True,timeout=(15,60),headers={"User-Agent":"History-Shorts-Pipeline/1.0"});r.raise_for_status();data=r.content
...
def fetch_clip_for_beat(query:str,fallback_query:str,out_dir:Path,index:int,duration:float|None=None,used_sources:set[str]|None=None,disabled_sources:set[str]|None=None)->Path|None:
    ...
    for attempt,(doc,url,size) in enumerate(candidates[:MAX_CANDIDATES],1):
        try:
    ...
    # Search multiple related queries and keep source diversity. A source that rate-limits is disabled for this run.
    items=[];seen_assets=set()
    for q in _query_variants(query)+_query_variants(fallback_query):
        if q.strip().lower() in {x.strip().lower() for x in (query,fallback_query or "")}:continue
        for item in search_all(q,cache):
            source=item.get("source","unknown");key=f"{source}:{item.get('url')}"
            if source in disabled_sources or key in seen_assets or key in used_sources or item.get("score",0)<4:continue
            seen_assets.add(key);items.append(item)
    # Round-robin again at the asset-consumption layer so one source cannot dominate.
    grouped={};
    ...
    for item in ordered:
        source=item.get("source","unknown");key=f"{source}:{item.get('url')}"
        try:
            p=_image_to_video(item["url"],out_dir/f"raw_{index}.mp4",duration or 5.0,index);used_sources.add(key);log.info("Using %s related historical visual for beat %d: %s",source,index,item.get("title"));return p
        except requests.HTTPError as exc:
            if getattr(exc.response,"status_code",None)==429:
                rate_limits+=1;disabled_sources.add(source);log.warning("%s rate-limited; disabling it for this run",source)
                if rate_limits>=MAX_WIKIMEDIA_429:break
            else:log.warning("%s visual failed: %s",source,exc)
